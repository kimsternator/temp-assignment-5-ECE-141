...
namespace ECE141 {
 ...
 // using cspace = Location (*)(const Location &aLocation, int aScale);
  ...
  static const size_t kMaxSteps = 700; //max steps before game-over
...
//    std::cout << "q5av(" << temp.col <<"," << temp.row << ")" << std::endl;
  ...
  public:
    ...
    void showSeperator(std::ostream &anOutput, const char *aPrefix="") {
      anOutput << aPrefix;
      for(int i=0;i<scale;i++) anOutput << "---|";
      anOutput << std::endl;
    }
    ...
    Reasons run(Player &aPlayer1, Player &aPlayer2, std::ostream &anOutput) {
      ...
      while(Reasons::tbd==reason && gameCanContinue(*activePlayer)) {
        ...
        if(!activePlayer->takeTurn(*this, orientation, anOutput)) {
          ...
          loserColor = activePlayer->color; //should've multi-jumped!
          reason=Reasons::missedJump;
        }
  ...
  //-- USE: game factory  -------------------------
  Reasons Game::run(Player &aP1, Player &aP2, std::ostream &anOut) {
    RealGame theGame;
